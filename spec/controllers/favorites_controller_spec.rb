   ...
   context 'signed in user' do
     ...
     describe 'POST create' do
       ...
       it 'creates a favorite for the current user and specified post' do
         expect(my_user_2.favorites.find_by_post_id(my_post.id)).to be_nil
         post :create, params: { post_id: my_post.id }
         expect(my_user_2.favorites.find_by_post_id(my_post.id)).not_to be_nil
       end
     end

    describe 'DELETE destroy' do
       it 'redirects to the posts show view' do
         favorite = my_user_2.favorites.where(post: my_post).create
         delete :destroy, params: { post_id: my_post.id, id: favorite.id }
         expect(response).to redirect_to([my_topic, my_post])
       end
